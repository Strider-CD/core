...
class MongooseCollection {
  ...
  save (item) {
    ...
    if (!validation.error) {
      return this.model.collection.insert(item).then(function (mongoReply) {
  ...
  update (id, item) {
    ...
    var self = this
    /*
    return self.model.findOneAndUpdate({'id': id}, item).then(function (res) {
      console.log('childNo: ', res)
      console.log('updated: ', flat(item))
      console.log('diff', compareJSON(item, res))
    ...
    */
    return self.model.findOne({'id': id}).then(function (res) {
      Object.assign(res, item)
      return self.model.update({'id': id}, res).then(function (raw) {
        return Promise.resolve(res.id)
      }).then(null, function (err) {
        return Promise.reject({
          message: errorMsg,
  ...
  findOneByQuery (query) {
    ...
    return this.model.findOne(query).then(function (res) {
      if (res) {
  ...
  findByQuery (query) {
    ...
    return this.model.find(query).then(function (res) {
      if (res) {
        return Promise.resolve(res)
      }
    }).then(null, function (err) {
